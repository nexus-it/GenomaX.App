...
<div class="form-group">
  ...
  <input name="txt_Ingreso<?php echo $NumWindow; ?>" type="text" id="txt_Ingreso<?php echo $NumWindow; ?>" size="10" onkeypress="BuscarIng<?php echo $NumWindow; ?>(event);" />
    ...
    <button class="btn btn-success" type="button" data-toggle="modal" data-target="#GnmX_Search" data-whatever="Ingreso" onclick="javascript:CargarSearch('Ingreso', 'txt_Ingreso<?php echo $NumWindow; ?>', 'Estado_ADM=*F*');"><i class="fas fa-search"></i></button>
   </span>
   </div>
</div>
 ...
 <div class="form-group"> 
  ...
  <input name="txt_fechaadm<?php echo $NumWindow; ?>" type="date" disabled="disabled" id="txt_fechaadm<?php echo $NumWindow; ?>" size="10" maxlength="10">
</div>

 <div class="form-group"> 
  <label for="txt_horaadm<?php echo $NumWindow; ?>">Hora</label>
  <input name="txt_horaadm<?php echo $NumWindow; ?>" type="time" disabled="disabled" id="txt_horaadm<?php echo $NumWindow; ?>" size="8" maxlength="8"><br />
</div>

<div class="form-group">  
  <label for="txt_Fecha<?php echo $NumWindow; ?>">Fecha Autorizaci&oacute;n</label>
  <input name="txt_Fecha<?php echo $NumWindow; ?>" type="date" id="txt_Fecha<?php echo $NumWindow; ?>" size="10" />
</div>

<div class="form-group">  
  <label for="txt_Motivo<?php echo $NumWindow; ?>">No. Autorizaci&oacute;n</label>
  <input name="txt_Motivo<?php echo $NumWindow; ?>" type="text" id="txt_Motivo<?php echo $NumWindow; ?>" size="20" />
</div>
...
<?php
  ...
  $Temp='<table class="table table-bordered table-condensed table-striped"><tr><th>FACTURA</th><th scope="col">FECHA</th><th scope="col">VAL. PCTE.</th><th scope="col">VAL. ENTIDAD</th></tr>';
...
from czterceros a, gxadmision b, czterceros c, gxeps d, gxplanes e, gxtipoingreso f, gxcausaexterna cxt where cxt.Codigo_CXT=b.Codigo_CXT and e.Codigo_PLA=b.Codigo_PLA and a.Codigo_TER=b.Codigo_TER and d.Codigo_TER=c.Codigo_TER and (b.Codigo_EPS)=(d.Codigo_EPS) and b.Ingreso_ADM=f.Tipo_ADM and b.Estado_ADM='F' and LPAD(Codigo_ADM,10,'0')=LPAD('".$_GET["Ingreso"]."',10,'0')";
  $result = mysqli_query($conexion, $SQL);  
  ...
  ";
    ...
    echo "
    MsgBox1('Actualizar Autorizaciones','El ingreso ".$_GET["Ingreso"]." no se encuentra facturado');
    ";
  }
  mysqli_free_result($result); 
  } else {
...
function BuscarIng<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
    AbrirForm('application/forms/autorizaciones.php', '<?php echo $NumWindow; ?>', '&Ingreso='+document.getElementById('txt_Ingreso<?php echo $NumWindow; ?>').value);
  }
}
