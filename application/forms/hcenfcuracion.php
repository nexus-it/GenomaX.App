			...
			?>
					  <div id="divnotaenf<?php echo $NumWindow; ?>" class="col-md-12">
						<div class="form-group" id="grp_txt_notaenf<?php echo $NumWindow; ?>">
							...
							<textarea class="form-control" rows="5" id="txt_notaenf<?php echo $NumWindow; ?>" name="txt_notaenf<?php echo $NumWindow; ?>" maxlength="5000" required>Curacion </textarea>
						</div>
					  </div>
					</div>
...
function BuscarHCPte<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
	if (document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value=="") {
		AbrirForm('application/forms/hcenfcuracion.php', '<?php echo $NumWindow; ?>', '');
	} else {
		AbrirForm('application/forms/hcenfcuracion.php', '<?php echo $NumWindow; ?>', '&Historia='+document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value);
	}  
  }
}
...
<script src="functions/nexus/hcenfcuracion.js?v=<?php echo $_SESSION["VERSION_CONTROL"]; ?>.<?php echo $NumWindow; ?>"></script>