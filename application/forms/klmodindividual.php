...
<?php 
	if (isset($_GET["PLA"])) {	
		...
		$SQL="Select Dias_PLA, individual_pla from klplanesprecios a, klplanes b Where a.Codigo_PLA=b.Codigo_PLA and Nombre_PLA='".str_replace('_', ' ', $_GET["PLA"])."'";
		$result = mysqli_query($conexion, $SQL);
...
function BuscarPLA<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
  	...
  	str=str.replace(/ /g, "_");
	AbrirForm('application/forms/klmodindividual.php', '<?php echo $NumWindow; ?>', '&PLA='+str);
   }
}
