...
<form action="" method="post" name="frm_form<?php echo $NumWindow; ?>" class="form-horizontal col-md-12" id="frm_form<?php echo $NumWindow; ?>"  enctype="multipart/form-data" onreset="KlResetea<?php echo $NumWindow; ?>();">
		...
		<div class="input-group">
			<input name="txt_plan<?php echo $NumWindow; ?>" id="txt_plan<?php echo $NumWindow; ?>" type="text" onkeypress="BuscarPLA<?php echo $NumWindow; ?>(event);" />
			 <span class="input-group-btn"> 		
	 		  <button class="btn btn-success" type="button" data-toggle="modal" data-target="#GnmX_Search" data-bs-toggle="modal" data-bs-target="#GnmX_Search" data-whatever="Planes" onclick="javascript:CargarSearch('KlPlanes', 'txt_plan<?php echo $NumWindow; ?>', 'Estado_PLA=*1*');"><i class="fas fa-search"></i></button>
			 </span>
		</div>
	</div>
	...
	<div class="form-group" id="grp_txt_idhc1<?php echo $NumWindow; ?>">
  		<label for="txt_dia<?php echo $indiv.$NumWindow; ?>">Dia <?php echo $indiv; ?></label>
...
<?php 
	if (isset($_GET["PLA"])) {	
		...
		$SQL="Select Dias_PLA, Hijos_pla from klplanesprecios a, klplanes b Where a.Codigo_PLA=b.Codigo_PLA and Nombre_PLA='".str_replace('_', ' ', $_GET["PLA"])."'";
		$result = mysqli_query($conexion, $SQL);
...
function BuscarPLA<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
  	...
  	str=str.replace(/ /g, "_");
	AbrirForm('application/forms/klmodfamilia.php', '<?php echo $NumWindow; ?>', '&PLA='+str);
   }
}
