	  		...
	  		<div class="well row">
	  			...
	  			<?php
					...
					<div class="form-group">
						...
						<input type="hidden" name="hdn_glax<?php echo $NumWindow; ?>" id="hdn_glax<?php echo $NumWindow; ?>" value="15">
					</div>
				</div>
				...
				<div class="col-md-12">
	  			...
	  			<div class="col-md-12">
	  				<label class="label label-success"> Escala Glasgow [ <b><span id="glasgow<?php echo $NumWindow; ?>" style="font-size:14; font-weight:bold;">15/15</span></b> ]</label>
	  					...
	  					<table  width="100%" align="center" cellpadding="1" cellspacing="2" bgcolor="#EFEFEF" class="table table-striped table-condensed tblDetalle table-bordered" id="tblliqadm<?php echo $NumWindow; ?>" >
							<tbody id="tbDetalle<?php echo $NumWindow; ?>">
								<tr id="trh<?php echo $NumWindow; ?>"> 
									...
									<th id="th2<?php echo $NumWindow; ?>">Glasgow</th> 
...
function CalcGlasgow<?php echo $NumWindow; ?>() {
	...
	document.getElementById("hdn_glax<?php echo $NumWindow; ?>").value=(Total);
}

function AddGlucom<?php echo $NumWindow; ?>() {
  	hora=document.getElementById("txt_horagluc<?php echo $NumWindow; ?>").value;
  	glax=document.getElementById("hdn_glax<?php echo $NumWindow; ?>").value;
	...
	celda2.innerHTML = '<input name="hdn_dantecedente'+TotalFilas+'<?php echo $NumWindow; ?>" type="hidden" id="hdn_dantecedente'+TotalFilas+'<?php echo $NumWindow; ?>" value="'+glax+''+'" /> '+glax; 
	celda3.innerHTML = '<input name="hdn_dantecedente'+TotalFilas+'<?php echo $NumWindow; ?>" type="hidden" id="hdn_dantecedente'+TotalFilas+'<?php echo $NumWindow; ?>" value="'+obs+''+'" /> '+obs; 
...
function BuscarHCPte<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
	if (document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value=="") {
		AbrirForm('application/forms/hcenfneuro.php', '<?php echo $NumWindow; ?>', '');
	} else {
		AbrirForm('application/forms/hcenfneuro.php', '<?php echo $NumWindow; ?>', '&Historia='+document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value);
	}  
  }
}
...
<script src="functions/nexus/hcenfneuro.js?v=<?php echo $_SESSION["VERSION_CONTROL"]; ?>.<?php echo $NumWindow; ?>"></script>