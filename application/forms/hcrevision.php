					...
					while($rowhc = mysqli_fetch_array($resulthc)) 
						{
							$contarow=$contarow+1;
							echo '
					  <tr onclick="ReportPreview'.$NumWindow.'(\'application/reports/hc.php?HISTORIA='.$rowhc["ID_TER"].'&FOLIO_INICIAL='.$rowhc["Folio_HCF"].'&FOLIO_FINAL='.$rowhc["Folio_HCF"].'\', \''.$rowhc["Folio_HCF"].'\');"><td align="center">'.$rowhc["Folio_HCF"].'</td><td align="center">'.formatofecha($rowhc[1]).'</td><td>'.$rowhc[6].'</td><td>'.$rowhc[2].'</td></tr>
					  ';
						}
...
function BuscarHCPte<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
	if (document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value=="") {
		AbrirForm('application/forms/hcrevision.php', '<?php echo $NumWindow; ?>', '');
	} else {
		AbrirForm('application/forms/hcrevision.php', '<?php echo $NumWindow; ?>', '&Historia='+document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value);
	}  
  }
}
