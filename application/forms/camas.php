...
<form action="" method="post" name="frm_form<?php echo $NumWindow; ?>" id="frm_form<?php echo $NumWindow; ?>" class="form-horizontal">
	...
	<div class="form-group">
		<label for="cmb_pabellon<?php echo $NumWindow; ?>">Pabellón</label>
		<select name="cmb_pabellon<?php echo $NumWindow; ?>" id="cmb_pabellon<?php echo $NumWindow; ?>">
		<?php 
	$SQL="Select Codigo_PBC, Descripcion_PBC from gxpabelloncamas order by 2";
...
<div class="form-group">
	<label for="cmb_ocupada<?php echo $NumWindow; ?>">Ocupada</label>
	<select name="cmb_ocupada<?php echo $NumWindow; ?>" id="cmb_ocupada<?php echo $NumWindow; ?>">
...
<div class="form-group">
	...
	<label class="label label-success">
		<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> CAMAS
	</label>
	<div class="row well well-sm">
		<div class="col-md-12">
			<div id="zero_detalle<?php echo $NumWindow; ?>" class="detalleord table-responsive "  >
				...
				<?php 
				$SQL="Select Codigo_CAM, Nombre_CAM, Descripcion_CAM, Descripcion_GRC, Descripcion_PBC, Nombre_ARE, Ocupada_CAM, Estado_CAM from gxcamas a, gxareas b, gxpabelloncamas c, gxgrupocamas d where c.Codigo_PBC=a.Codigo_PBC and c.Codigo_GRC=d.Codigo_GRC and a.Codigo_ARE=b.Codigo_ARE Order By 1 ";
				$contafila=0;
				$result = mysqli_query($conexion, $SQL);
				while ($row = mysqli_fetch_array($result)) {
					if ($row[6]=="0") {
					...
					<td>'.$row[4].'</td>
					<td>'.$row[5].'</td>
					<td align="center"> <span class="glyphicon glyphicon-'.$ocupad.'" aria-hidden="true"></span> </td>
...
<?php
	if (isset($_GET["CodigoCAM"])) {
		...
		";
	$SQL="Select Codigo_CAM, Nombre_CAM, Descripcion_CAM, Codigo_ARE, Codigo_PBC, Ocupada_CAM, Estado_CAM from gxcamas a where Codigo_CAM='".$_GET["CodigoCAM"]."'";
	$result = mysqli_query($conexion, $SQL);
	if($row = mysqli_fetch_array($result)) {
		echo "
			...
			document.frm_form".$NumWindow.".cmb_ocupada".$NumWindow.".value='".$row[5]."';
			document.frm_form".$NumWindow.".cmb_pabellon".$NumWindow.".value='".$row[4]."';
...
function ReLoad<?php echo $NumWindow; ?>(Consult) {
	AbrirForm('application/forms/camas.php', '<?php echo $NumWindow; ?>', '&CodigoCAM='+Consult);
}

function Save<?php echo $NumWindow; ?>() {
	Guardar_camas('<?php echo $NumWindow; ?>');
}
