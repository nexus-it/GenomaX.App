			 	...
			 	if (trim($_GET["LaFactura"])!="") {
					...
					while($rowhc = mysqli_fetch_array($resulthc)) 
					  ...
					  <tr >
					  	...
					  	<td align="right"><input  name="txt_totserv'.$contarow.$NumWindow.'" id="txt_totserv'.$contarow.$NumWindow.'" type="number" min="0" max="'.$rowhc[4].'" class="izq form-control" required disabled value="0" style="height:24px"/></td>
					  </tr>
					  ';
						}
	...
	if (isset($_GET["LaFactura"])) {
		if (trim($_GET["LaFactura"])!="") {
			...
			echo "
				...
				echo "MsgBox1('Notas Creditos','No es posible realizar NC a la factura ".$LaFactura."');";
			}
			mysqli_free_result($result); 
...
function calcvalores<?php echo $NumWindow; ?>() {
	...
	do {
	    cantservx= parseInt(document.getElementById('txt_cantserv'+contafilas+'<?php echo $NumWindow; ?>').value);
	    cantantx=parseInt(document.getElementById('hdn_cantant'+contafilas+'<?php echo $NumWindow; ?>').value);
	    valservx=parseInt(document.getElementById('hdn_valserv'+contafilas+'<?php echo $NumWindow; ?>').value);
	    if (isNaN(cantservx)) {
	    	MsgBox1("Error","Valor ingresado en cantidad a acreditar debe ser numerico. Fila "+contafilas);
	    	document.getElementById('txt_cantserv'+contafilas+'<?php echo $NumWindow; ?>').value=0;
	    } else {
	    	if (cantservx > cantantx) {
	    		MsgBox1("Error","Valor ingresado en cantidad a acreditar no puede ser mayor a la cantidad facturada. Fila "+contafilas+ " ["+cantservx+">"+cantantx+"]");
	    		document.getElementById('txt_cantserv'+contafilas+'<?php echo $NumWindow; ?>').value=cantantx;
	    	} else {
	    		document.getElementById('txt_totserv'+contafilas+'<?php echo $NumWindow; ?>').value=cantservx*valservx;
	    		totalnc=totalnc+(cantservx*valservx);
	    	}
	    }
	    contafilas++;
	}
	while (contafilas <= totalfilas);
	...
	document.getElementById('txt_valornc<?php echo $NumWindow; ?>').value=totalnc;
	document.getElementById('txt_valfactnew<?php echo $NumWindow; ?>').value=valfac-totalnc;

	document.frm_form<?php echo $NumWindow; ?>.txt_observacion<?php echo $NumWindow; ?>.value='NC A FACTURA '+numfact+' POR VALOR DE $ '+totalnc;
}
...
function CargarFact<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
	if (document.getElementById('txt_factura<?php echo $NumWindow; ?>').value!="") {
		...
		AbrirForm('application/forms/notascredito.php', '<?php echo $NumWindow; ?>', '&LaFactura='+LaFactura);
	}  
  }
}
