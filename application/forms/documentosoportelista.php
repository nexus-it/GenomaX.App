...
<div class="container">
  <form action="" method="post" name="frm_form<?php echo $NumWindow; ?>">
	<label class="label label-default">Listado de Documento soporte</label>
  <div class="row well well-sm">
   
    <div class="col-md-2">
	<div class="form-group" id="grp_txt_idhc1<?php echo $NumWindow; ?>">
		<label for="txt_factura<?php echo $NumWindow; ?>">Documento soporte</label>
    ...
    <div class="col-md-2">
	<div class="form-group" id="grp_txt_idhc1<?php echo $NumWindow; ?>">
		<label for="txt_paciente<?php echo $NumWindow; ?>">Proveedor</label>
    ...
    <span class="input-group-btn">	
      ...
      <div class="container"> 
        <button class="btn btn-success" type="button" onclick="CargarForm('application/forms/documentosoporte.php', 'Documento Soporte', 'money_add.png');">Nuevo Documento Soporte <i class="fa fa-file"></i></button>
  		</div>
		</div>
</form>
<?php
echo '<table class="table table-striped table-condensed tblDetalle table-bordered">'; 
			$conteo = listarDocumentoSoporte($filtro,$ini,$fin);
echo '</table>';
...
function RefreshList<?php echo $NumWindow; ?>() {
  ...
  AbrirForm('application/forms/documentosoportelista.php', '<?php echo $NumWindow; ?>', '&page=1&numfact='+numfact+'&fechaini='+fechaini+'&fechafin='+fechafin+'&paciente='+paciente+'&contrato='+contrato);
}

function filtrarDocSop(filtro){
    $.ajax({
        type: 'POST',
        url: 'application/forms/documentosoportelista.php',
        data: {
            filtro: filtro
        },
        beforeSend: function()
...
filtrarDocSop($("#filtro<?php echo $NumWindow; ?>").val());
      });
});
...
$(document).ready(function() {	
    $( ".pagefact" ).click(function() {
      var newpage = $(this).attr('data');
		  AbrirForm('application/forms/documentosoportelista.php', '<?php echo $NumWindow; ?>', '&page='+newpage);
    });
});
...
function putSendFactura(factura){
  ...
  showProgress("1", factura)
   ...
   function descargaFactXml(factura){
      $.ajax({
        type: 'POST',
        url: 'functions/php/GenomaXBackend/descargaFactXml.php',
        data: {
          factura: factura
        },
        beforeSend: function()
