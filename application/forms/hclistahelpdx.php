					...
					while($rowhc = mysqli_fetch_array($resulthc)) 
						{
							$contarow=$contarow+1;
							echo '
					  <tr onclick="ReportPreview'.$NumWindow.'(\'application/reports/hcayudasdx.php?HISTORIA='.$rowhc["ID_TER"].'&FOLIO_INICIAL='.$rowhc[1].'&FOLIO_FINAL='.$rowhc[1].'\', \''.$rowhc[1].'\');"><td align="center">'.$rowhc[0].'</td><td align="center">'.$rowhc[1].'</td><td align="center">'.$rowhc[2].'</td><td>'.$rowhc[3].'</td><td>'.$rowhc[4].'</td></tr>
					  ';
						}
...
function BuscarHCPte<?php echo $NumWindow; ?>(e) {
  ...
  if (tecla==13){
	if (document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value=="") {
		AbrirForm('application/forms/hclistahelpdx.php', '<?php echo $NumWindow; ?>', '');
	} else {
		AbrirForm('application/forms/hclistahelpdx.php', '<?php echo $NumWindow; ?>', '&Historia='+document.getElementById('txt_idhc<?php echo $NumWindow; ?>').value);
	}  
  }
}
