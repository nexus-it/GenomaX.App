...
<form action="" method="post" name="frm_form<?php echo $NumWindow; ?>" class="form-horizontal" id="frm_form<?php echo $NumWindow; ?>" >
	...
	<?php 
		$SQL="SELECT a.Codigo_CNS, a.Nombre_CNS FROM gxconsultorios a, gxareas b WHERE a.Codigo_ARE=b.Codigo_ARE AND a.Estado_CNS='1' AND b.Estado_ARE='1' AND b.Admisiones_ARE='1' ORDER BY 2;";
...
<?php
	if  (((!(isset($_GET["modulo"]))) && ($Contamodulos==1))||(isset($_GET["modulo"]))) {
		$SQL="SELECT a.Codigo_CNS, a.Nombre_CNS FROM gxconsultorios a, gxareas b WHERE a.Codigo_ARE=b.Codigo_ARE AND a.Estado_CNS='1' AND b.Estado_ARE='1' AND b.Admisiones_ARE='1' and Codigo_CNS='".$Elmodulo."';";
		$result = mysqli_query($conexion, $SQL);
		while($row = mysqli_fetch_array($result)) 
		{
	    	$Nombremodulo=$row[1];
	...
	<div class=""> 
		<a href="javascript:RefreshTurno<?php echo $NumWindow; ?>();" role="button" class="btn btn-primary btn-sm btn-block" title="Actualizar"> <span class="glyphicon glyphicon-refresh" aria-hidden="true"></span> Actualizar Listado</a>
	</div>
...
function RefreshTurno<?php echo $NumWindow; ?>() {
	AbrirForm('application/forms/turnos_adm.php', '<?php echo $NumWindow; ?>', '&modulo=<?php echo $Elmodulo; ?>&contamodulos=<?php echo $Contamodulos; ?>');
}
...
function selmodulo<?php echo $NumWindow; ?>() {
	AbrirForm('application/forms/turnos_adm.php', '<?php echo $NumWindow; ?>', '&modulo='+document.getElementById('cmb_modulo<?php echo $NumWindow; ?>').value+'&contamodulos='+document.getElementById('hdn_contamodulos<?php echo $NumWindow; ?>').value);	
}

function ChngMod<?php echo $NumWindow; ?>() {
	AbrirForm('application/forms/turnos_adm.php', '<?php echo $NumWindow; ?>', '');	
}
