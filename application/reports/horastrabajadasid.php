...
$this->Cell(0,5,'TOTAL HORAS POR EMPLEADO ENTRE EL '.$_GET["FECHA_INICIAL"].' Y EL '.$_GET["FECHA_FINAL"],'B',0,'C',0);
$this->SetY(10);
$this->SetFont('Arial','',8);
...
$Orientation="P";
$NombreEmpresa="";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt, RazonSocial_DCD from ".$_SESSION['DB_NXS'].".itreports, itconfig where codigo_rpt='horastrabajadasid'";
...
Where G.Codigo_MRC=B.Codigo_MRC AND A.Codigo_TER=B.Codigo_TER AND C.Codigo_CRG=D.Codigo_CRG AND D.Codigo_TER=A.Codigo_TER AND F.Codigo_TER=A.Codigo_TER AND F.Codigo_TCL=E.Codigo_TCL 
 AND B.Fecha_IDM>='@FECHA_INICIAL 00:00:00' AND B.Fecha_IDM<='@FECHA_FINAL 23:59:59' AND A.Nombre_TER like '%@EMPLEADO%' AND D.FechaFin_CRG ='0000-00-00' 
...
$pdf->Cell(40,4,"FECHA",'TBR',0,'C',1); //ENC FECHA
$pdf->Cell(40,4,"HORA",'TBRL',0,'C',1); //ENC HORA
$pdf->Cell(80,4,"TIPO DE MARCACION",'TBL',0,'C',1); //ENC TIPO MARCACION
$pdf->Cell(0,4,"TOTAL HORAS",'TBL',0,'C',1); //ENC TIPO MARCACION
$pdf->Ln();

//SE CONSULTAN LAS FECHAS DE MARCACION DEL USUARIO
$SQL="Select distinct date(B.Fecha_IDM)
From czterceros A, idmarcaciones B, czcargos C, czcargoemp D, cztipocontratos E, czempleados F, idtipomarcacion G 
Where G.Codigo_MRC=B.Codigo_MRC AND A.Codigo_TER=B.Codigo_TER AND C.Codigo_CRG=D.Codigo_CRG AND D.Codigo_TER=A.Codigo_TER AND F.Codigo_TER=A.Codigo_TER AND F.Codigo_TCL=E.Codigo_TCL 
 AND B.Fecha_IDM>='@FECHA_INICIAL 00:00:00' AND B.Fecha_IDM<='@FECHA_FINAL 23:59:59' AND A.Nombre_TER like '%@EMPLEADO%' AND D.FechaFin_CRG ='0000-00-00' AND A.ID_TER='".$row[0]."'
...
while ($rowfecha = mysqli_fetch_row($resfecha)) {
	...
	$SQL="Select distinct date_format(B.Fecha_IDM, '%h:%i:%s %p'), G.Nombre_MRC, Case Codigo_USR When '-' Then ' ' Else '*' End, B.Fecha_IDM 
	From czterceros A, idmarcaciones B, czcargos C, czcargoemp D, cztipocontratos E, czempleados F, idtipomarcacion G 
	Where G.Codigo_MRC=B.Codigo_MRC AND A.Codigo_TER=B.Codigo_TER AND C.Codigo_CRG=D.Codigo_CRG AND D.Codigo_TER=A.Codigo_TER AND F.Codigo_TER=A.Codigo_TER AND F.Codigo_TCL=E.Codigo_TCL 
	 AND B.Fecha_IDM>='@FECHA_INICIAL 00:00:00' AND B.Fecha_IDM<='@FECHA_FINAL 23:59:59' AND A.Nombre_TER like '%@EMPLEADO%' AND D.FechaFin_CRG ='0000-00-00' AND A.ID_TER='".$row[0]."'
	 AND date(B.Fecha_IDM)='".$rowfecha[0]."' AND G.Tipo_MRC='1'
	Order By 4 Asc Limit 1";
	$SQL=str_replace("@FECHA_INICIAL",($_GET["FECHA_INICIAL"]),$SQL);
	$SQL=str_replace("@FECHA_FINAL",($_GET["FECHA_FINAL"]),$SQL);
	$SQL=str_replace("@EMPLEADO",$_GET["EMPLEADO"],$SQL);
	$resmarca = mysqli_query($conexion, $SQL);
	if ($fechita!=$rowfecha[0]) {
		...
		$fechita=$rowfecha[0];
		$top="T";
		$pdf->Cell(40,4,FormatoFecha($fechita),'RT',0,'C',$fondito); //FECHA
	} else {
		$pdf->Cell(40,4,'','R',0,'C',$fondito); //FECHA
		$top="";
	}
	if ($rowmarca = mysqli_fetch_row($resmarca)) {
		$pdf->Cell(5,4,$rowmarca[2],'L'.$top,0,'C',$fondito);
		$pdf->Cell(35,4,$rowmarca[0],'R'.$top,0,'C',$fondito);
		$pdf->Cell(80,4,$rowmarca[1],'L'.$top,0,'L',$fondito);
		$pdf->Cell(0,4,'','L'.$top,0,'C',$fondito);
		$entrada=$rowmarca[0];
	} else {
		$pdf->Cell(5,4,'','L'.$top,0,'C',$fondito);
		$pdf->Cell(35,4,'00:00:00','R'.$top,0,'C',$fondito);
		$pdf->Cell(80,4,'SIN ENTRADA','L'.$top,0,'L',$fondito);
		$pdf->Cell(0,4,'','L'.$top,0,'C',$fondito);
		$entrada='00:00';
	}
	mysqli_free_result($resmarca);
	$pdf->Ln();
	//MARCACION DE SALIDA
	$SQL="Select distinct date_format(B.Fecha_IDM, '%h:%i:%s %p'), G.Nombre_MRC, Case Codigo_USR When '-' Then ' ' Else '*' End, B.Fecha_IDM 
	From czterceros A, idmarcaciones B, czcargos C, czcargoemp D, cztipocontratos E, czempleados F, idtipomarcacion G 
	Where G.Codigo_MRC=B.Codigo_MRC AND A.Codigo_TER=B.Codigo_TER AND C.Codigo_CRG=D.Codigo_CRG AND D.Codigo_TER=A.Codigo_TER AND F.Codigo_TER=A.Codigo_TER AND F.Codigo_TCL=E.Codigo_TCL 
	 AND B.Fecha_IDM>='@FECHA_INICIAL 00:00:00' AND B.Fecha_IDM<='@FECHA_FINAL 23:59:59' AND A.Nombre_TER like '%@EMPLEADO%' AND D.FechaFin_CRG ='0000-00-00' AND A.ID_TER='".$row[0]."'
	 AND date(B.Fecha_IDM)='".$rowfecha[0]."' AND G.Tipo_MRC='2'
	...
	if ($rowmarca = mysqli_fetch_row($resmarca)) {
		...
		$pdf->Cell(35,4,$rowmarca[0],'R'.$top,0,'C',$fondito);
		$pdf->Cell(80,4,$rowmarca[1],'L'.$top,0,'L',$fondito);
		$dif=date("H:i:s", strtotime("00:00:00") + strtotime($rowmarca[0]) - strtotime($entrada) );
		$pdf->Cell(0,4,$dif,'L'.$top,0,'C',$fondito);
	} else {
		$pdf->Cell(5,4,'','L'.$top,0,'C',$fondito);
		$pdf->Cell(35,4,'23:59:59','R'.$top,0,'C',$fondito);
		$pdf->Cell(80,4,'SIN SALIDA','L'.$top,0,'L',$fondito);
		$dif=date("H:i:s", strtotime("00:00:00") + strtotime('23:59:59') - strtotime($entrada) );
		$pdf->Cell(0,4,$dif,'L'.$top,0,'C',$fondito);
	}
	mysqli_free_result($resmarca);
	$pdf->Ln();
...
$pdf->SetTextColor(0);
$pdf->Cell(0,4,'TOTAL '.utf8_decode($row[1]).': '.number_format($totalhoras,2).' HORAS','B',0,'R',1);
