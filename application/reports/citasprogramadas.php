...
$this->Cell(8,6,'No.','R',0,'C',1);
$this->Cell(11,6,'HORA','R',0,'C',1);
$this->Cell(23,6,'DOCUMENTO','R',0,'C',1);
$this->Cell(62,6,'PACIENTE','R',0,'C',1);
$this->Cell(9,6,'EDAD','R',0,'C',1);
$this->Cell(8,6,'SEXO','R',0,'C',1);
$this->Cell(28,6,'TELEFONO','R',0,'C',1);
$this->Cell(33,6,'DIRECCION','R',0,'C',1);
$this->Cell(46,6,'ENTIDAD','',0,'C',1);
$this->Cell(20,6,'REGISTRA','',0,'C',1);
$this->Cell(0,6,'FECHA','',0,'C',1);
$this->SetY(40);
}
function Footer()
{
...
$Orientation="P";
$NombreEmpresa="";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt, RazonSocial_DCD from ".$_SESSION['DB_NXS'].".itreports, itconfig where codigo_rpt='citasprogramadas'";
...
if ($row = mysqli_fetch_row($result)) {
	...
	$Orientation=$row[2];
	$NombreEmpresa=$row[3];
    $SQL=str_replace("@VARFECHA",strtolower($_GET["VARFECHA"]),$SQL);
...
while($row = mysqli_fetch_row($result)) {
    ...
    if ($Concat!=$OtraPage) {
        ...
        $pdf->Cell(0,5,''.utf8_decode($row[3]),'',0,'R',0);  
        $pdf->SetY(20);
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(100,5,'Area: '.utf8_decode($row[1]),'',0,'L',0);      
        $pdf->Cell(0,5,''.utf8_decode($row[4]),'',0,'R',0);  
        $pdf->Ln();    
...
$pdf->Cell(8,6,$totalpte,'',0,'C',1); //INGRESO
$pdf->Cell(11,6,utf8_decode($row[6]),'',0,'C',1); //FECHA
$pdf->Cell(23,6,utf8_decode($row[7].' '.$row[8]),'',0,'L',1); //DOCUMENTO
$pdf->Cell(62,6,utf8_decode($row[9]),'',0,'L',1); //PACIENTE
$pdf->Cell(9,6,utf8_decode($row[10]),'',0,'C',1); //TIPO
$pdf->Cell(8,6,utf8_decode($row[11]),'',0,'C',1); //TIPO
$pdf->Cell(28,6,utf8_decode($row[12]),'',0,'L',1); //ENTIDAD
$pdf->Cell(33,6,utf8_decode($row[13]),'',0,'L',1); //ESTADO
$pdf->Cell(46,6,utf8_decode($row[14]),'',0,'L',1); //ESTADO
$pdf->Cell(20,6,utf8_decode($row[15]),'',0,'L',1); //ESTADO
$pdf->Cell(0,6,utf8_decode(FormatoFecha($row[16])),'',0,'C',1); //ESTADO
$pdf->Ln();
}
mysqli_free_result($result);
//mysqli_close();
