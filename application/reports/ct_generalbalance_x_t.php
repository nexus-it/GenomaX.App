...
$Orientation="P";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt from ".$_SESSION['DB_NXS'].".itreports where codigo_rpt='ct_generalbalance_x_t'";
...
$pdf->Settitle('BALANCE GENERAL POR TERCEROS');
...
while($row = mysqli_fetch_row($result)) {
    ...
    switch ($Nivel) {
        ...
        case "5":
            ...
            $taB=16;
            $pdf->Cell($taB,$Line,'','',0,'L',0);
            $pdf->Cell(150-$taB,$Line,utf8_decode($row[6]." -- ".$row[11]),'',0,'L',0);
        break;
    }
    //$pdf->Cell($taB,$Line,'','',0,'L',0);
    //$pdf->Cell(150-$taB,$Line,utf8_decode($row[6]." -- ".$row[11]),'',0,'L',0);
