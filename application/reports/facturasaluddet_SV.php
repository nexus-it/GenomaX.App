...
if(isset($_GET["DB_HOST"])) {
	$conexion = mysqli_connect($_GET["DB_HOST"], $_GET["DB_USER"], $_GET["DB_PASSWORD"], $_GET["DB_NAME"], $_GET["DB_PORT"]);
...
if(isset($_GET["namedoc"])) {
	//$pdf->Output();
	$pdf->Output($_SERVER["DOCUMENT_ROOT"]."/functions/php/GenomaXBackend/sendmails/archivos/FES-".$_GET["namedoc"].".pdf", "F");
} else {
	$pdf->Output();
}
/*
$destination_folder = '../../functions/php/GenomaXBackend/sendmails/archivos/';
$url = 'https://enterprise.genomax.app/application/reports/'."FES-".$_GET["namedoc"].".pdf";
