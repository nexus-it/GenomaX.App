...
$Orientation="P";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt from ".$_SESSION['DB_NXS'].".itreports where codigo_rpt='ctmovcont'";
$result = mysqli_query($conexion, $SQL);
if ($row = mysqli_fetch_row($result)) {
    $SQL=$row[0];
    $Orientation=$row[2];
    $FormatoPagina=$row[1];
    $SQL=str_replace("@CNT1",$_GET["CNT1"],$SQL);
    $SQL=str_replace("@CNT2",$_GET["CNT2"],$SQL);
}
...
$pdf->Settitle('MOVIMIENTO CONTABLE');
