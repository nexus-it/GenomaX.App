...
$this->Cell(0,10,'LISTADO DE POLIZAS EN STANDBY EMITIDAS ENTRE EL '.$_GET["FECHA_INICIAL"].' Y EL '.$_GET["FECHA_FINAL"],'B',0,'C',0);
$this->SetY(10);
$this->SetFont('Arial','',9);
...
$this->Cell(30,5,'Agencia','BR',0,'C',0);
$this->Cell(18,5,'Plan','BR',0,'C',0);
$this->Cell(13,5,'Voucher','BR',0,'C',0);
$this->Cell(7,5,'Edad','BR',0,'C',0);
$this->Cell(11,5,'Telefono','BR',0,'C',0);
$this->Cell(18,5,'Correo','BR',0,'C',0);
...
$Orientation="P";
$NombreEmpresa="";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt, RazonSocial_DCD from ".$_SESSION['DB_NXS'].".itreports, itconfig where codigo_rpt='kstandby'";
...
if ($row = mysqli_fetch_row($result)) {
	...
	$Orientation=$row[2];
	$NombreEmpresa=$row[3];
	$SQL=str_replace("@DIASV",($_GET["DIASV"]),$SQL);
...
$pdf->Settitle('REPORTE POLIZAS EN STANDBY POR PERIODO');
...
$pdf->Cell(30,4,utf8_decode($row[5]),'',0,'L',1); //ENTIDAD
...
$pdf->Cell(11,4,$row[9],'',0,'L',1); //ESTADO
$pdf->Cell(18,4,strtolower($row[10]),'',0,'L',1); //ESTADO
$pdf->SetFont('Arial','',7);
$pdf->Cell(13,4,ucwords(strtolower(utf8_decode($row[11]))),'',0,'R',1); //ESTADO
$pdf->Cell(14,4,FormatoFecha($row[12]),'',0,'R',1); //ENTIDAD
$pdf->Cell(14,4,FormatoFecha($row[13]),'',0,'L',1); //ENTIDAD
$pdf->Cell(6,4,$row[14],'',0,'L',1); //ENTIDAD
$pdf->Cell(16,4,utf8_decode($row[15]),'',0,'L',1); //ENTIDAD
$pdf->Cell(18,4,utf8_decode($row[16]),'',0,'L',1); //ENTIDAD
$pdf->Cell(10,4,$row[17],'',0,'R',1); //ENTIDAD
$pdf->Cell(0,4,utf8_decode(substr($row[18],0,stripos($row[18],'['))),'',0,'L',1); //ENTIDAD
// $pdf->Cell(0,4,number_format($row[20],0,'.',','),'',0,'R',1); //ENTIDAD
$pdf->Ln();
$totalent=$totalent+$row[17];
$totalfacts++;
}
mysqli_free_result($result);
//mysqli_close();
$pdf->SetFont('Arial','B',9);
$pdf->Cell(180,6,"Polizas StandBy emitidas en el periodo: ".$totalfacts,'T',0,'L',1); //ESTADO
