...
$Orientation="P";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt from ".$_SESSION['DB_NXS'].".itreports where codigo_rpt='ct_incomestatement'";
...
while($row = mysqli_fetch_row($result)) {
    ...
    if ($row[0]!=$Empresa) {
        ...
        $pdf->Cell(0,6,utf8_decode('Estado de resultados desde '.$_GET["FECHA_INICIAL"].' hasta '.$_GET["FECHA_FINAL"]),'B',0,'C',0);
        $pdf->SetY(9);
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Ln();
