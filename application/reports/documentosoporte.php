...
$pdf->SetFont('Arial','B',9);
$pdf->Cell(26,5,'Valor Doc. Sop.: ','',0,'L',0);
$pdf->SetFont('Courier','B',11);
$pdf->Cell(41,5,'$ '.number_format($row[8],2,'.',','),'',0,'L',0); //valor NC
$pdf->Ln();
$pdf->SetFont('Arial','B',9);
$pdf->Cell(25,5,'Retencion: ','',0,'L',0);
$pdf->SetFont('Arial','',9);
$pdf->Cell(90,5,utf8_decode($row[17]).' - '.utf8_decode($row[18]),'',0,'L',0); //Retencion
$pdf->SetFont('Arial','B',9);
$pdf->Cell(26,5,'Valor Retencion.: ','',0,'L',0);
$pdf->SetFont('Courier','B',11);
$pdf->Cell(41,5,'$ '.number_format($row[8]*($row[18]/100),2,'.',','),'',0,'L',0); //valor retencion 
$pdf->Ln();
$pdf->SetFont('Arial','B',9);
$pdf->Cell(25,5,' ','',0,'L',0);
$pdf->SetFont('Arial','',9);
$pdf->Cell(90,5,'','',0,'L',0); //Retencion
$pdf->SetFont('Arial','B',9);
$pdf->Cell(26,5,'Valor Total.: ','',0,'L',0);
$pdf->SetFont('Courier','B',11);
$pdf->Cell(41,5,'$ '.number_format($row[8]-($row[8]*($row[18]/100)),2,'.',','),'',0,'L',0); //valor NC
...
$pdf->SetFont('Arial','',9);
$pdf->MultiCell(0,5,utf8_decode(strtoupper(ValorLetras($row[8]-($row[8]*($row[18]/100))))),0); //Descripcion
$pdf->SetFont('Arial','B',9);
...
//if($_GET["TIPO_NOTA"] == "C"){
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(0,3,utf8_decode('Detalle Documento Soporte '),'T',0,'R',0);
        $pdf->Ln();
        ...
        $SQL="Select id, id, producto, valor, cantidad, (valor * cantidad) From gxdocumentosoporte where factura ='".$row[0]."' ";
