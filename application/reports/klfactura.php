...
function Footer()
{
    ...
    $this->Cell(100,5,' [Kl´ud] Powered by Nexus-it.co','T',0,'L');
...
while ($rowH = mysqli_fetch_row($resultH)) {
	...
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(0,5,$rowH[34].' - '.$rowH[35],'BR',0,'L',0);	
	$pdf->Ln();
...
//	echo $SQL;
	...
	$pdf->Cell(0,5,number_format($rowH[14]+$rowH[15]+$rowH[13],2,'.',','),'TBR',0,'R',0);
	$pdf->Ln();
	$pdf->SetFont('Arial','',10);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(163,5,'VALOR DESCUENTO','LBR',0,'R',0);
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(5,5,'$','TB',0,'C',0);
