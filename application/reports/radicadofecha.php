...
$this->Cell(0,10,'LISTADO DE FACTURAS RADICADAS ENTRE EL '.$_GET["FECHA_INICIAL"].' Y EL '.$_GET["FECHA_FINAL"],'B',0,'C',0);
$this->SetY(10);
$this->SetFont('Arial','',9);
$this->Cell(0,7,'Usuario: '.$_SESSION["it_CodigoUSR"].' - '.$_SESSION["it_user"],'',0,'R',0); //Codigo Usuario
$this->SetY(32);
$this->SetFont('Arial','B',8);
$this->Cell(14,6,'RADICA','BR',0,'C',0);
$this->Cell(15,6,'F. RAD','BR',0,'C',0);
...
$Orientation="P";
$NombreEmpresa="";
$SQL="SELECT sql_rpt, page_rpt, orientacion_rpt, RazonSocial_DCD from ".$_SESSION['DB_NXS'].".itreports, itconfig where codigo_rpt='radicadofecha'";
...
$pdf->Settitle('LISTADO FACTURAS RADICADAS POR PERIODO');
...
$pdf->Cell(14,5,$row[0],'',0,'R',1); //ENTIDAD
$pdf->Cell(15,5,$row[1],'',0,'L',1); //ENTIDAD
$pdf->Cell(23,5,$NoFact,'',0,'C',0); //INGRESO
$pdf->Cell(15,5,$row[3],'',0,'C',0); //FECHA
$pdf->Cell(18,5,$row[4],'',0,'C',0); //DOCUMENTO
$pdf->Cell(24,5,$row[5],'',0,'L',0); //PACIENTE
$pdf->Cell(48,5,ucwords(strtolower($row[6])),'',0,'L',1); //TIPO
$pdf->Cell(20,5,$row[7],'',0,'L',1); //ENTIDAD
$pdf->Cell(32,5,$row[9],'',0,'L',1); //ESTADO
$pdf->Cell(17,5,$row[10],'',0,'L',1); //ESTADO
$pdf->Cell(18,5,number_format($row[11],0,'.',','),'',0,'R',1); //ESTADO
$pdf->Cell(0,5,number_format($row[12],0,'.',','),'',0,'R',1); //ESTADO
$pdf->Ln();
$totalpte=$totalpte+$row[9];
$totalent=$totalent+$row[14];
$totalfacts++;
}
mysqli_free_result($result);
//mysqli_close();
$pdf->SetFont('Arial','B',9);
$pdf->Cell(180,6,"Facturas radicadas en el periodo: ".$totalfacts,'T',0,'L',1); //ESTADO
