  ...
  $result = mysqli_query($conexion, $SQL);
	if($row = mysqli_fetch_array($result)) {
      ...
      while($row = mysqli_fetch_array($result)){
            ...
            $btnxml='onclick="descargaFactXml(\''.$row[5].'\', \''.$row[0].'\'); "';
...
function listarNotasDebito($filtro,$ini,$fin){
  ...
  $SQL="SELECT * FROM gxfacturas t1   INNER JOIN gxadmision t2 ON t1.Codigo_ADM = t2.Codigo_ADM  INNER JOIN czterceros t3 ON t3.Codigo_TER = t2.Codigo_TER   INNER JOIN gxeps t4 ON t2.Codigo_EPS = t4.Codigo_EPS  INNER JOIN cznotascontablesenc t5 ON T1.Codigo_FAC = T5.NumeroDoc_NCT and t5.Naturaleza_NCT = 'D'";
   //$SQL .=  " where T1.codigo_fac= 'BQ-14414'  "; 
