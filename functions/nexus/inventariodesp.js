...
function DespMedSol2(Ventana, FiltroArea, FiltroPcte, FiltroFecIni, FiltroFecFin) {
	document.getElementById('zero_detalle'+Ventana).innerHTML='...';
	$.get(Funciones,{'Func':'DespMedSol2','filtroarea':FiltroArea, 'filtropcte':FiltroPcte,'filfecini':FiltroFecIni,'filfecfin':FiltroFecFin,'ventana':Ventana},function(data){ 
		document.getElementById('zero_detalle'+Ventana).innerHTML=data;
	});
}

function invdesppac(NumSol, Ventana) {
  CargarWind('Solicitud a farmacia ['+NumSol+'] ', 'forms/inventariodesp2.php?numsol='+NumSol, '1.Pills.png', 'inventariosolfarm.php',Ventana );
}

function NewSolFarm(NumSol, Ventana) {
  CargarWind('Nueva Solicitud a Farmacia ', 'forms/inventariodesp2.php?numsol='+NumSol, '1.Pills.png', 'inventariosolfarm.php',Ventana );
}
