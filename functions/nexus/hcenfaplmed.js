...
function Guardar_hcenfaplmed(Ventana) 
{
	xError="";
	...
	if (document.getElementById('txt_nombreserv'+Ventana).value=="ERROR: No se encuentra el servicio") {
		tError="Cambio de Dispositivo"
		xError="El codigo no pertenece a un dispositivo válido";
		document.getElementById('txt_nombreserv'+Ventana).focus;
	}
	if (document.getElementById('hdn_controwMed'+Ventana).value!="0") {
		Total=document.getElementById('hdn_controwMed'+Ventana).value;
		Aplic=0;
		for (i = 1; i <= Total; i++) {
			if (Aplic=="0") {
				Aplic=document.getElementById('txt_cantaplcadam'+i+Ventana).value;
			}
		}
		if (Aplic!=0) {
			if (document.getElementById('hdn_diezc'+Ventana).value!="0") {
				tError="Aplicación de Medicamentos"
				xError="No ha seleccionado los 10 correctos. ";
			}
		}
	}
	...
	if (xError=="") {
		$.ajax({  
		  type: "POST",  
		  url: Transact +"hcenfaplmed.php",  
		  data: "Func=hcenfaplmed&"+RecorrerForm($("#frm_form"+Ventana)),  
